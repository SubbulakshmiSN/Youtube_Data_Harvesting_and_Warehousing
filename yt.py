...
if selected == "Home":
    ...
    st.info("""
               #### About this page
             - **This application is  user friendly and intends to provide users with the ability to access and analyse data from numerous YouTube channels**.
             - **Google API, SQL, MongoDB, and Streamlit are used  to develop a user-friendly application that allows users to retrieve, save, and query YouTube channel and video data**.
            """)
...
if selected == "Extract & Transform":
    ...
    with tab1:
        ...
        if ch_id and st.button("Extract Data"):
            existing_channel = db.channel_details.find_one({"Channel_id": ch_id[0]})
            if existing_channel:
                st.warning(f"Channel with ID {ch_id[0]} is already extracted for '{existing_channel['Channel_name']}'.")
